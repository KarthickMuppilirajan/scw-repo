...
$resumeoptions = array(
    ...
    'maxbytes' => $CFG->maxbytes,
    'maxfiles' => 1,
    'accepted_types' => array('document','.pdf')
);
...
$args["addflag1"] = $addflag1;
$args["editoroptions"] = $editoroptions;
$args['resumeoptions'] = $resumeoptions;
...
if(!empty($userdetails)){
    ...
    $regtxt = $editusers->awards_reg_editor['text'];
	$regtxt = str_replace("brokenfile.php#","draftfile.php",$regtxt);
	$editusers->awards_reg_editor['text'] = $regtxt;	
	...
	if($userdetails->profession==1){
	    ...
	    $editusers->industry_others = $userdetails->industry_others;
	    $editusers->functional_area_others = $userdetails->functional_area_others;
	}
...
if ($usernew = $userform->get_data()) {
    ...
    if(!empty($userdetails)){
		$industry_others=$usernew->industry_others;
		$functional_area_others=$usernew->functional_area_others;
        ...
        $userdetails->awards_regformat = 1;
		...
		if($userdetails->profession==2){
			...
			file_save_draft_area_files($usernew->resume, $syscontext->id, 'local_scwbanner', 'resume',
                   $userdetails->user_id, $resumeoptions);
        }
		
		if($userdetails->profession==1){
			$userdetails->industry = implode(',', $usernew->industry);
            $userdetails->functional_area =  implode(',', $usernew->functional_area); 
            $usernew->industry_others =  ($usernew->industry=="other") ? $usernew->industry_others : '';
            $userdetails->industry_others = $industry_others;
			$usernew->functional_area_others =  ($usernew->functional_area=="other") ? $usernew->functional_area_others : '';
            $userdetails->functional_area_others = $functional_area_others;
        }
        ...
        if($userdetails->profession==1){
			for($ci=1;$ci<=$userdetails->no_company;$ci++){
                ...
                $udata->currently_working = isset($usernew->{"currently_working".$ci}) ? $usernew->{"currently_working".$ci} : 0;
	...
	$msg = ($addflag==1 || $addflag1==1)? '' : 'Successfully updated';
...
require(['jquery'], function( $ ) {
	$("input.cwork").change(function(){
        $('input.cwork').not(this).prop("checked", false);      
	});

	$("#id_addmorebutton").on('click',function(e){
		e.preventDefault();
		var data=$(".companyinfo-box :input").serialize();
		console.log(data);
		$(this).prop("disabled", true);
		var url = M.cfg.wwwroot+'/user/ajx-add-more.php';
		var rurl = M.cfg.wwwroot+'/user/editprofile.php';
		$.ajax({
			method:"POST",
			url: url,
			data: data+"&addmore=Addmore",
			async:false
		}).done(function(msg){
	...
	$(".btn-remove").on('click', function(e){
		...
		$.ajax({
			method:"POST",
			url: url,
			data: "removebtn=Remove&removerow="+dr,
			async:false
		}).done(function(msg){
			var pmsg = JSON.parse(msg);
		     window.onbeforeunload = null;
	...
	$("#id_addmorebutton1").on('click',function(e){
		e.preventDefault();
		var data=$(".educationinfo-box :input").serialize();
		var url = M.cfg.wwwroot+'/user/ajx-add-more.php';
		var rurl = M.cfg.wwwroot+'/user/editprofile.php';
		
		var data1 = $(".companyinfo-box :input").serialize();
		
		$.ajax({
			method:"POST",
			url: url,
			data: data + "&addmore1=Addmore&" + data1,
			async:false
		}).done(function(msg){
			var pmsg = JSON.parse(msg);
		     window.onbeforeunload = null;
    ...
    $(".btn-remove1").on('click', function(e){
		...
		$.ajax({
			method:"POST",
			url: url,
			data: "removebtn1=Remove&removerow1="+dr,
			async:false
		}).done(function(msg){
			var pmsg = JSON.parse(msg);
		     window.onbeforeunload = null;
	...
	//Industry change
    $(document).on('change', "#id_industry", function(){
      var cmbind = $("#id_industry option").filter(":selected").val();
      if(cmbind=="other"){
        $("#fitem_id_industry_others").show();
		$("#id_industry_others").val('');
      }else{
        $("#fitem_id_industry_others").hide();
		$("#id_industry_others").val('test');
	  }
    });
	
	//Industry Others onload
    var cmbind1 = $("#id_industry option").filter(":selected").val();
	...
	$(document).on('change', "#id_functional_area", function(){
      var cmbind = $("#id_functional_area option").filter(":selected").val();
      if(cmbind=="other"){
        $("#fitem_id_functional_area_others").show();
		$("#id_functional_area_others").val('');
      }else{
        $("#fitem_id_functional_area_others").hide();
		$("#id_functional_area_others").val('test');
	  }
    });
	
	//Functional Area Others
    var cmbind2 = $("#id_functional_area option").filter(":selected").val();
