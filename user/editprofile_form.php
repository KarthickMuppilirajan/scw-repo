    ...
    public function definition () {
        ...
        if (isset($userdetails->profession) && $userdetails->profession==1) {
            for($i=1;$i<=$userdetails->no_company;$i++){
				...
				$mform->addRule($fld, 'Please enter valid Job Title', 'regex', '/^[a-zA-Z-., ]+$/', 'client');
                ...
                $mform->addRule($fld, $strmissingfield, 'required', null, 'client');
				$mform->addRule($fld, 'Please enter valid Company', 'regex', '/^[a-zA-Z-., ]+$/', 'client');
                $mform->setType($fld, PARAM_TEXT);
        ...
        if(!empty($userdetails)){
            if ($userdetails->profession==1) {
		        ...
		        $select=$mform->addElement('select', 'industry', get_string('industry', 'local_scwgeneral'), $industries);
				...
				$mform->setType('industry_others', PARAM_TEXT);
				
		        $fareas = local_scwgeneral_get_farea();
                $fareas = array('' => get_string('selectafarea', 'local_scwgeneral') . '...') + $fareas;
		        $select=$mform->addElement('select', 'functional_area', get_string('farea', 'local_scwgeneral'), $fareas);
				$select->setMultiple(true);
				...
				$mform->setType('functional_area_others', PARAM_TEXT);
            }
        }
        ...
        if(!empty($userdetails)){
			...
			if($userdetails->profession==2){
				...
				$mform->addElement('text', 'sniff_keyword', get_string('sniff_keyword'), ' maxlength="100" class="textfield"');
				$strmissingfield = 'Please enter Searchable Profile';
		        $mform->addRule('sniff_keyword', $strmissingfield, 'required', null, 'client');
				$mform->addHelpButton('sniff_keyword', 'sniff_keyword');
				$mform->setType('sniff_keyword', PARAM_TEXT);
        ...
        if(!empty($userdetails)){
			...
			$strmissingfield = 'Please enter Website/Blog';
			$mform->addRule('web_url', 'Please enter valid Website/Blog', 'regex', 
			  '/^(http|https):\/\/(\w+:{0,1}\w*@)?(\S+)(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?/', 'client');
			
			$mform->addElement('static', 'static_web_url',null, 'Please enter Website/Blog like this http://example.com or http://www.example.com');
			...
			$mform->addRule('linkedin_url', 'Please enter valid linked in URL', 'regex', 
			  '/^(ftp|http|https):\/\/?((www|\w\w)\.)?linkedin.com(\w+:{0,1}\w*@)?(\S+)(:([0-9])+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?/', 'client');
			
            $mform->setType('linkedin_url', PARAM_RAW);
			
			$mform->addElement('static', 'static_linkedin_url',null, 'Please enter Linked in url like this https://www.linkedin.com/in/profileName');
    ...
    public function validation($data, $files) {
        ...
        if(!empty($userdetails)){
            if ($userdetails->profession==1) {
                ...
                for($i=1;$i<=$userdetails->no_company;$i++){
                    $from_month = isset($data['from_month'.$i])?$data['from_month'.$i]:null;
                    $to_month = isset($data['to_month'.$i])?$data['to_month'.$i]:null;
                    $from_year = isset($data['from_year'.$i])?$data['from_year'.$i]:null;
                    $to_year = isset($data['to_year'.$i])?$data['to_year'.$i]:null;
            ...
            for($i=1;$i<=$userdetails->no_education;$i++) {
				$from_year = isset($data['edu_from_year'.$i])? $data['edu_from_year'.$i] : null;
                $to_year = isset($data['edu_to_year'.$i])? $data['edu_to_year'.$i] : null;
