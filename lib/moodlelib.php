...
//    $data->link      = $CFG->httpswwwroot .'/login/forgot_password.php?token='. $resetrecord->token;
    $data->link      = $CFG->httpswwwroot .'/login/set_password.php?token='. $resetrecord->token;
    $data->admin     = generate_email_signoff();
    ...
    // Directly email rather than using the messaging system to ensure its not routed to a popup or jabber.
    return email_to_user($user, $supportuser, $subject, $message, $messagehtml); //modified by praburaj for email template (march 9,2017)

}

function send_password_change_confirmation_email_new($user, $resetrecord) {
    global $CFG;
...
//    $data->link      = $CFG->httpswwwroot .'/login/forgot_password.php?token='. $resetrecord->token;
    ...
    $message = get_string('emailresetconfirmation_new', '', $data);
    $subject = get_string('emailresetconfirmationsubject', '', format_string($site->fullname));
