...
/*function core_login_process_password_reset_request_new($data) {
    global $DB, $OUTPUT, $CFG, $PAGE;
    $systemcontext = context_system::instance();
    if (0) {
        redirect(get_login_url());
...
//			echo '<pre>';
    ...
    echo $OUTPUT->header();
	echo '<div class="login-blk">';
	echo '<div class="loginbox clearfix twocolumns scwloginarea-blk">';
    echo $OUTPUT->box(get_string('passwordforgotteninstructions2'), 'generalbox boxwidthnormal boxaligncenter');
    $mform->display();
	echo '</div>';
	echo '</div>';
...
//		echo $OUTPUT->box(get_string('setpasswordinstructions'), 'generalbox boxwidthnormal boxaligncenter');
		echo $OUTPUT->box(get_string('setpasswordinstructions'), 'boxwidthnormal boxaligncenter');
		echo '<br><div class="subcontent loginsub">';
		$mform->display();
		echo '</div>
		</div>
