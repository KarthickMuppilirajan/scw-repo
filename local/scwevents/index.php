...
$PAGE->set_url('/local/scwevents/index.php');
...
$sparams = array();
$baseurl =  new moodle_url('/local/scwevents/index.php', $sparams);
$seg = $page;
$perpage = '5';		
$offset  = $seg*$perpage;
if(isset($_REQUEST['keyword']))
{
	...
	$sql="SELECT * FROM {local_scwevents} WHERE event_delete =0 and event_status = '1' AND (event_startdate > $time or event_enddate > $time) and event_name like '%$keyword%' ORDER BY event_startdate ASC";
	$csql="SELECT COUNT('x') FROM {local_scwevents}  WHERE event_delete =0 and event_status = '1' AND (event_startdate > $time or event_enddate > $time) and event_name like '%$keyword%'";
}
else
{	
	$sql="SELECT * FROM {local_scwevents} WHERE event_delete =0 and event_status = '1' AND (event_startdate > $time or event_enddate > $time) ORDER BY event_startdate ASC";
	$csql="SELECT COUNT('x') FROM {local_scwevents}  WHERE event_delete =0 and event_status = '1' AND (event_startdate > $time or event_enddate > $time)";
}
$totalcount = $DB->count_records_sql($csql);

$paging = $OUTPUT->paging_bar($totalcount , $page , $perpage, $baseurl);

$result = $DB->get_records_sql("$sql", $sparams, $offset, $perpage);
//$result = $DB->get_records_sql($sql);
if(empty($result))
{
	echo '<div class="eventlist-contnt"> <div class="eventlist-contnt"> <div class="row clearfix"> <div class="alert alert-info"> <strong>No Records Found.</strong></div></div></div></div>';	
}
else
{
...
{	
	...
	$event_time=date('h.i a',$obj->event_startdate).' UTC';
...
<h5><a id="event<?php echo $id?>" href="<?php echo $url?>"><?php echo $event_name;?></a><span><a href="<?php echo $url?>"><button class="btn btn-sm btn-ashblk pull-right">View More</button></span></a></h5>
