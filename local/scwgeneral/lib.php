...
function local_scwgeneral_cron(){
    ...
    // Banner Flush
    $DB->execute("UPDATE {local_scwbanner} SET banner_delete='1' WHERE banner_created <= '$time' and banner_expires_by < $today ");

    // Interviews Flush
    $DB->execute("UPDATE {local_scwinterviews} SET interview_delete='1' WHERE interview_created <= '$time' and interview_expires_by < $today ");

    //Videos
    $DB->execute("UPDATE {local_scwvideos} SET video_delete='1' WHERE video_created <= '$time' ");

	// Events Flush
    $DB->execute("UPDATE {local_scwevents} SET event_delete='1' WHERE event_created <= '$time' and event_enddate < $today ");	
}